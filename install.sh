...
download_config() {
    ...
    # Priority 1: Use environment variables (most secure)
    if [[ -n "$CONFIG_GIST_ID" ]] && [[ -n "$GITHUB_TOKEN" ]]; then
        LOGI "使用环境变量下载配置 (Gist ID: ${CONFIG_GIST_ID:0:8}...)"
        local gist_url="https://gist.githubusercontent.com/Michaol/${CONFIG_GIST_ID}/raw/config.json"
        wget --header="Authorization: token ${GITHUB_TOKEN}" \
             --no-check-certificate \
             -O "${CONFIG_FILE_PATH}/config.json" \
             "$gist_url" 2>&1 | grep -v "Authorization"
        
        if [[ $? -ne 0 ]]; then
            LOGE "从 Gist 下载配置失败"
            LOGE "请检查 GITHUB_TOKEN 和 CONFIG_GIST_ID 是否正确"
            exit 1
        else
            LOGI "配置下载成功"
            return 0
    ...
    # Priority 2: Use URL parameter
    if [[ -n "$config_url" ]]; then
        LOGI "从远程 URL 下载配置"
        
        # Check if URL needs authentication
        if [[ -n "$GITHUB_TOKEN" ]] && [[ "$config_url" =~ gist\.githubusercontent\.com ]]; then
            wget --header="Authorization: token ${GITHUB_TOKEN}" \
                 --no-check-certificate \
                 -O "${CONFIG_FILE_PATH}/config.json" \
                 "$config_url" 2>&1 | grep -v "Authorization"
        else
    ...
    # Priority 3: Try to find config in current directory
    if [[ -f "./config.json" ]]; then
        LOGI "发现本地配置文件 config.json，正在导入..."
        cp "./config.json" "${CONFIG_FILE_PATH}/config.json"
    elif [[ -f "./server_config.json" ]]; then
        LOGI "发现本地配置文件 server_config.json，正在导入..."
        cp "./server_config.json" "${CONFIG_FILE_PATH}/config.json"
    else
        if [[ ! -f "${CONFIG_FILE_PATH}/config.json" ]]; then
            LOGE "未找到配置文件"
            LOGE ""
...
#install sing-box, parameters: $1=version (optional), $2=config_url (optional)
install_sing-box() {
    set_as_entrance
    LOGD "开始安装sing-box..."
    ...
    # Parse parameters
    if [[ $# -eq 1 ]]; then
        # Single parameter could be version or URL
        if [[ "$1" =~ ^https?:// ]]; then
            config_url="$1"
        else
            version="$1"
...
show_help() {
    ...
    echo "sing-box install      - 直接安装 sing-box 服务"
